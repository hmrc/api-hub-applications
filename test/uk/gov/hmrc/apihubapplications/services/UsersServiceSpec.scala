...
class UsersServiceSpec
  ...
  "findAll" - {
    "must return an empty list if no emails are found in the repo" in {
      ...
      when(fixture.searchService.findAll(None, true)).thenReturn(Future.successful(Seq.empty))
    ...
    "must remove duplicate emails from list ignoring domain case" in {
      ...
      when(fixture.searchService.findAll(None, true)).thenReturn(Future.successful(applications))
