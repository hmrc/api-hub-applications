...
class ApplicationsCredentialsServiceSpec extends AsyncFreeSpec with Matchers with MockitoSugar with TableDrivenPropertyChecks with EitherValues {
  ...
  "addCredential" - {
    ...
    "must create a new secondary credential and call ScopeFixer" in {
      ...
      val updatedApp = app
        .addCredential(Secondary, expectedCredential)
      ...
      service.addCredential(testAppId, FakeHipEnvironments.secondaryEnvironment)(HeaderCarrier()) map {
        newCredential =>
    ...
    "must create a new primary credential and call ScopeFixer where the existing credential is not hidden" in {
      ...
      val updatedApp = app
        .addCredential(Primary, expectedCredential)
        .copy(lastUpdated = LocalDateTime.now(clock))
      ...
      service.addCredential(testAppId, FakeHipEnvironments.primaryEnvironment)(HeaderCarrier()) map {
        newCredential =>
    ...
    "must return ApplicationCredentialLimitException if there are already 5 credentials" in {
      ...
      service.addCredential(applicationId, FakeHipEnvironments.primaryEnvironment)(HeaderCarrier()).map {
        result =>
          result mustBe Left(ApplicationCredentialLimitException.forApplication(application, FakeHipEnvironments.primaryEnvironment))
      }
    }
  }
  ...
  "deleteCredential" - {
    "must delete the correct credential from the correct environment" in {
      ...
      forAll(hipEnvironments) {(hipEnvironment: HipEnvironment) =>
        ...
        service.deleteCredential(applicationId, hipEnvironment, clientId)(HeaderCarrier()).map {
          result =>
            verify(idmsConnector).deleteClient(eqTo(hipEnvironment.environmentName), eqTo(clientId))(any)
            result mustBe Right(())
        }
      }
    }
    ...
    "must update the application in the repository" in {
      ...
      service.deleteCredential(applicationId, FakeHipEnvironments.primaryEnvironment, clientId)(HeaderCarrier()).map {
        result =>
  ...
  "fetchAllScopes" - {
    "must return the correct credential scopes" in {
      ...
      when(hipEnvironments.environments).thenReturn(Seq(FakeHipEnvironments.primaryEnvironment, FakeHipEnvironments.secondaryEnvironment))
