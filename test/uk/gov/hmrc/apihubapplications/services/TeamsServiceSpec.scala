...
class TeamsServiceSpec
  ...
  "create" - {
    "must transform NewTeam to Team, pass it to the repository, and return the saved result" in {
      ...
      fixture.service.create(newTeam, "TODO")(any).map {
        result =>
  ...
  "addTeamMember" - {
    "must add the new team member to the team" in {
      ...
      fixture.service.addTeamMember(id, TeamMemberRequest(teamMember3.email), "TODO")(HeaderCarrier()).map {
        result =>
    ...
    "must return TeamMemberExistsException if the team member already exists within the team" in {
      ...
      fixture.service.addTeamMember(id, TeamMemberRequest(teamMember1.email), "TODO")(HeaderCarrier()).map {
        result =>
  ...
  "removeTeamMember" - {
    "must remove the team member from the team" in {
      ...
      fixture.service.removeTeamMember(id, teamMember3.email, "TODO")(HeaderCarrier()).map {
        result =>
    ...
    "must prevent removing the last team member from the team" in {
      ...
      fixture.service.removeTeamMember(id, teamMember1.email, "TODO")(HeaderCarrier()).map {
        result =>
  ...
  "renameTeam" - {
    "must lookup the team, update it, and persist" in {
      ...
      fixture.service.renameTeam(id, RenameTeamRequest("new name"), "TODO").map {
        result =>
  ...
  "addEgressesToTeam" - {
    "must add the set of egresses to the team when the team has no egresses" in {
      ...
      fixture.service.addEgressesToTeam(id, request, "TODO").map {
        result =>
  ...
  "removeEgressFromTeam" - {
    "must remove the egress from the team when it exists" in {
      ...
      fixture.service.removeEgressFromTeam(id, egressId1, "TODO").map {
        result =>
    ...
    "must return TeamNotFoundException when the team does not exist in the repository" in {
      ...
      fixture.service.removeEgressFromTeam(id, egressId, "TODO").map {
        result =>
          verify(fixture.repository, never()).update(any)
          result mustBe Left(expected)
      }
