...
class CircuitBreakers @Inject()(
  ...
  given apimCircuitBreakers: CircuitBreakersType[ApimException] = EnvironmentName.values
    .map(environmentName =>
      environmentName -> APIMCircuitBreaker[EnvironmentName](servicesConfig, metricsService, environmentName)
    ).toMap

  given idmsCircuitBreaker: CircuitBreakersType[IdmsException] = EnvironmentName.values
    .map(environmentName =>
      environmentName -> IdmsCircuitBreaker[EnvironmentName](servicesConfig, metricsService, environmentName)
    ).toMap
