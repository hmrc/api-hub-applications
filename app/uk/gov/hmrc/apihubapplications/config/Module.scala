...
import uk.gov.hmrc.apihubapplications.connectors.{APIMConnector, APIMConnectorImpl, EmailConnector, EmailConnectorImpl, IdmsConnector, IdmsConnectorEnvironmentParityImpl, IdmsConnectorImpl, IntegrationCatalogueConnector, IntegrationCatalogueConnectorImpl}
...
class Module extends play.api.inject.Module {
  ...
  private def idmsConnectorBinding(configuration: Configuration): Class[? <: IdmsConnector] = {
    if (configuration.get[Boolean]("features.environment-parity")) {
      classOf[IdmsConnectorEnvironmentParityImpl]
    }
    else {
