...
trait HipEnvironments {
  ...
  def forUrlPathParameter(pathParameter: String): Option[HipEnvironment] =  
    environments.find(hipEnvironment => hipEnvironment.id == pathParameter || hipEnvironment.id == pathParameter)
...
class HipEnvironmentsImpl @Inject(configuration: Configuration) extends HipEnvironments {
  ...
  override val deploymentEnvironment: HipEnvironment = environments.sortBy(_.rank)(Ordering[Int].reverse).headOption
    .getOrElse(throw new IllegalArgumentException("No deployment environment found"))
