...
) extends DbModel[Environments] {

  override def toModel(dbApplication: DbApplication): Environments =
    Environments(
      primary = primary.toModel(dbApplication),
      secondary = secondary.toModel(dbApplication)
    )
