...
class SyntheticMonitoringScheduler @Inject()(
  ...
  private val idmsClientId: String = configuration.get[String]("microservice.services.idms-primary.clientId")

  given HeaderCarrier = HeaderCarrier()

  private def checkApimStatus(): Future[Unit] = {
    Future.sequence(
      EnvironmentName.values.map(environment =>
        Future.sequence(
          Seq(
            apimConnector.getDeployments(environment),
            idmsConnector.fetchClient(environment, idmsClientId),
          )
        )
      )
    ).map(_ => ())
